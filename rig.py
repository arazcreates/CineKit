...
def create_dolly(context, cam_obj):
    ...
    # Root sits at the camera; the path runs ±3 m along the camera's local X,
    # so ck_position=0.5 keeps the camera at its original spot.
    ...
    spline.points.add(1)
    spline.points[0].co = (-3.0, 0.0, 0.0, 1.0)
    ...
    con.use_curve_follow = False  # banking off by default
    ...
    aim.matrix_world = Matrix.Translation(
        cam_obj.matrix_world.translation + fwd * 5.0)

    _def_prop(root, "ck_position", 0.5, 0.0, 1.0,
              "Position along the dolly path (keyframeable)")
    _def_prop(root, "ck_track", 0.0, 0.0, 1.0,
...
def create_crane(context, cam_obj):
    ...
    base_loc = Vector((cam_loc.x, cam_loc.y, max(cam_loc.z - 1.5, 0.0)))
    ...
    _def_prop(root, "ck_boom", 45.0, -80.0, 80.0, "Boom up/down (degrees)")
    _def_prop(root, "ck_length", length0, 0.5, 20.0, "Arm length (metres)")
    _def_prop(root, "ck_tilt", 0.0, -90.0, 90.0,
...
def create_handheld(context, cam_obj, profile='DOCUMENTARY'):
    ...
    _def_prop(root, "ck_intensity", 1.0, 0.0, 2.0,
...
def create_orbit(context, cam_obj):
    ...
    _def_prop(root, "ck_speed", 30.0, -720.0, 720.0,
